  ...
  <div class="min-h-screen bg-[#f9f4ec] text-slate-950">
    ...
    <main class="mx-auto grid max-w-[1560px] gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
      <section class="rounded-2xl border border-orange-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
        <p class="text-xs font-bold uppercase tracking-[0.35em] text-[#d94b57]">Registro</p>
        ...
        <form class="mt-6 space-y-4" @submit.prevent="submitRegistration">
          <label class="block space-y-2">
            ...
            <input v-model="form.nombre" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="Jose Perez" />
          </label>

          <label class="block space-y-2">
            <span class="text-sm font-semibold text-slate-700">Correo</span>
            <input v-model="form.correo" type="email" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="cliente@correo.com" />
          </label>

          <label class="block space-y-2">
            <span class="text-sm font-semibold text-slate-700">Contrasena</span>
            <input v-model="form.password" type="password" autocomplete="new-password" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="Minimo 6 caracteres" />
          </label>

          <div class="grid gap-4 sm:grid-cols-2">
            <label class="block space-y-2">
              <span class="text-sm font-semibold text-slate-700">DNI</span>
              <input v-model="form.dni" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="12345678" />
            </label>
            <label class="block space-y-2">
              <span class="text-sm font-semibold text-slate-700">Telefono</span>
              <input v-model="form.telefono" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="999111222" />
            </label>
          </div>

          <label class="block space-y-2">
            <span class="text-sm font-semibold text-slate-700">Referencia de pago</span>
            <input v-model="form.referencia_pago" class="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none focus:border-[#d94b57]" placeholder="Operacion, voucher o codigo" />
          </label>
      ...
      <section class="space-y-4">
        <div class="rounded-2xl bg-[#d94b57] p-6 text-white">
        ...
        <article
          ...
          :class="form.plan === plan.id ? 'border-[#d94b57] ring-2 ring-[#d94b57]/20' : 'border-orange-100 hover:border-[#d94b57]/40'"
        ...
        >
          <div class="flex items-start justify-between gap-4">
            ...
            <p class="text-3xl font-black text-[#d94b57]">S/ {{ plan.price }}</p>
          </div>
          <div class="mt-4 flex flex-wrap gap-2">
            <span v-for="tag in plan.tags" :key="tag" class="rounded-full bg-[#fff4ea] px-3 py-1 text-xs font-bold uppercase tracking-[0.12em] text-slate-600">{{ tag }}</span>
          </div>
        </article>
