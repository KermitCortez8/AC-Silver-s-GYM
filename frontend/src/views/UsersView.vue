...
const handleSubmit = () => {
  const member = gymStore.upsertMember({ id: editingId.value || undefined, ...form });

  if (form.assignMembership) {
    gymStore.assignPlanToMember(member.id, form.planId, form.promotionId, form.membershipNote || 'Asignación/renovación desde expediente');
  }
